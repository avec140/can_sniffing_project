...
현대 차량은 ECU 간 통신을 위해 **CAN (Controller Area Network)** 프로토콜을 사용한다.  
하지만 CAN 통신은 인증(Authentication) 및 암호화(Encryption)가 존재하지 않는 구조적 특성을 가지며,  
메시지 위조(Message Injection) 및 재전송(Replay) 공격에 취약하다.
(지금은 보안 프로토콜 적용(SecOC)의 기술을 활용해 무결성을 보장하거나, CAN버스 상 IDS설치해 실시간으로 감지할 수 있도록 대책을 연구하고 있다.)
본 프로젝트는 실제 차량 없이 Linux **SocketCAN(vCAN)** 환경에서 차량 네트워크 공격을 재현하고 분석하는 것을 목표로 한다.
...
├── oneclick_attack.py # 공격 실행 관리 파일
├── can_generator.py # 정상 CAN 트래픽 생성
├── log_sniffer_logger.py # CAN 스니핑 및 CSV 저장
├── replay_attack.py # replay 공격 
├── injection_attack.py #비정상 패킷 injection 공격
├── fuzz_attack.py # fuzzing 공격
├── logs/
└── README.md
